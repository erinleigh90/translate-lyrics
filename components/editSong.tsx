...
import {
  ...
  dsinsertSong,
  editSong,
} from "../utilities/databaseCrudMethods";
...
export default function EditSong({
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      ...
      if (artistName) {
        let artistMatches: any = allArtists.filter(
          (artist: any) => artist.name == artistName
        );
        let artist: any;
      ...
      if (albumTitle) {
        let albumMatches: any = allAlbums.filter(
          (album: any) => album.title == albumTitle
        );
      ...
      if (song) {
        ...
        // DataStore (testing without artist and song):
        song = await dsinsertSong(songTitle, lyrics);
      }
